...
params = Create_Params(@(y,z) 0*y,@(y,z) 0*y,@(y,z) 1+0*z,1,1,0,0);
...
plot_mode = 3;  % mode number to show
if plot_mode ~= 0
    ...
    Plot_Mode(p_01(:,:,plot_mode),grid.y,grid.z,H(grid.lambda));
    Plot_Mode(p_1(:,:,plot_mode),grid.y,grid.z,H(grid.lambda));
    Plot_Mode(p_3(:,:,plot_mode),grid.y,grid.z,H(grid.lambda));
end
